...
export default function WritePageClient() {
  ...
  // Once a deletion would drop the total below zero, pull the doc-start
  // baseline up so zero is a floor — new writing after this point is
  // credited right away instead of first having to pay off the deficit.
  useEffect(() => {
    ...
    if (ratcheted !== docStartWordCount) {
      setDocStartWordCount(ratcheted);
    }
  }, [sessionStartWordCount, wordCount, docStartWordCount]);
